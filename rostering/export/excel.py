...
spreadsheet, this only affects that one row (e.g. one solved role, or
Vedoucí místností); the room header row and every other row still show
those rooms as separate columns. Row-block order top to bottom: the
building-wide structural roles (Vedoucí budovy, Pravá ruka), Vedoucí
místností (per room), the 6 solved roles (Opravovatel/Měnič/.../Fotograf;
Záloha is deferred to the bottom to match the historical layout), the
overlay roles (Uvaděči účastníků, Focení předávání cen, Registrace), then
Záloha and Technická podpora. See CLAUDE.md for the role glossary.
"""
...
def write_roster(
    ...
    # ---- Vedoucí místností: per room, with any of this row's own merges ----
    ...
    for b in buildings:
        for group in row_groups(StructuralRole.VedouciMistnosti.name, b.name, [r.name for r in b.rooms]):
            col_start, col_end = group_col_range(group, b.name)
            names = [n for room_name in group for n in structural_room.get((b.name, room_name), [])]
